...
//打开新增权利人界面的地址，需要配置
// var ADDRESS_ADDQLR = "http://192.168.50.127:8080/estateplat-cadastre/static/html/FWDZGL/addQlrxx.html";
...
$(function() {
  ...
  renderQueryResults(data, QUERY_FWXX, LZXX);
...
$("#linkDCXX").on("click", function() {
  ...
  renderQueryResults(data, QUERY_FWXX, DCXX);
});
...
$("#linkPMT").on("click", function() {
  ...
  //   "fwDcbIndex": fwDcbIndex
  // };
  // renderQueryResults(data, QUERY_FWDZHST, PMT);
  layer.msg("暂无此功能！");
});
...
function renderToTpl(mytpl, data) {
  ...
  if (r === 1) {
    $("#LZXX_FWJG option").each(function() {
      if ($(this).val() === data.fwjg) {
        $(this).attr('selected', true);
      }
    });

    $("#LZXX_FWYT option").each(function() {
      if ($(this).val() === data.fwyt) {
        $(this).attr('selected', true);
      }
    });
