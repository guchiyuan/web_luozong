define(["dojo/_base/declare",
    ...
    "map/core/BaseWidget",
    "map/widgets/RelatedBuildingTable/BuildingTableRenderer",
    "map/component/ChosenSelect",
    "text!map/widgets/RelatedBuildingTable/config.json",
    "static/thirdparty/laydate/laydate",
    "static/thirdparty/bootstrap/js/bootstrap-v3.min"], function (declare, lang, on, layer, Mustache, Handlebars, SlimScroll, laypage, Draw, Graphic, esriLang,
                                                                  QueryTask, MapUtils, MapPopup, JsonConverters, BaseWidget, ListDataRenderer, ChosenSelect,configJson) {
     ...
     */
    function _init() {
        $("#ImportScLpbSpatialQueryPanel").animate();
        ...
        laydate.skin('molv');
        //初始化jq变量
        $attrQueryPanel = $("#ImportScLpbQueryPanel");
        $qResultPanel = $("#ImportScLpbResultPanel");
        $spatialQueryPanel = $("#ImportScLpbSpatialQueryPanel");
        //读取弹出层方式
        ...
        $attrQueryPanel.append(renderTpl($("#ImportScLpbLayerSelectTpl").html(), { layers: _queryConfig.layers }));
        $lyrSelector = $("#ImportScLpbLayersSelect");
        $qSearchBtn = $("#ImportScLpbSearchBtn");
        //默认选中第一个图层
        ...
        $qSearchBtn.on('click', function () {
            var serviceId = $("#ImportScLpbLayersSelect").val();
            $qSearchBtn.text('查询中 ...');
                ...
                if (serviceId == (n.serviceId + "-" + n.layerIndex)) {
                    ...
                    if (inputVal.nodeName.toLowerCase() == 'input' || inputVal.nodeName.toLowerCase() == 'select') {
                        ...
                        if (inputVal.value != undefined && inputVal.value != '') {
                            if (where.length > 0 && index < $("#ImportScLpbLayerForm :input").length) {
                    ...
                    var data = {
                        ...
                        page: 1,
                        size: parseInt(($(window).height() - 280) / 75, 10)
                    };
                    $.ajax({
                        url: "/omp/map/query",
                        data: data,
        ...
        //清空重置监听
        $("#ImportScLpbResetBtn").on('click', function () {
            var _selItem = $lyrSelector.val();
            $("#ImportScLpbLayerForm")[0].reset();
    ...
    function changeActiveLayer(sid) {
        ...
        $.each(_queryConfig.layers, function (index, layer) {
            ...
            if (sid === tid) {
                ...
                $("#ImportScLpbLayerCondition").empty();
                $("#ImportScLpbLayerCondition").append(renderTpl($("#ImportScLpbSearchTpl").html(), { fields: layer.queryFields.fields }));
                return false;
            }
        });
    }
    ...
    function parseQueryResult(r) {
        ...
        if (rp == undefined || rp.features == undefined) {
            ...
            $.each(_queryConfig.layers, function (n, layerItem) {
                if ($lyrSelector.val() == (layerItem.serviceId + "-" + layerItem.layerIndex)) {
                    ...
                    $.each(rp.features, function (i, feature) {
                        ...
                        var titleText = esriLang.substitute({ alias: tf.alias, value: feature.attributes[tf.name] || '空' }, '${alias}:${value}');
                        var subTitleText1 = sf.hasOwnProperty('title1')?esriLang.substitute({ alias: sf.title1.alias||'空', value: feature.attributes[sf.title1.name] || '空' }, '${alias}:${value}'):'';
                        var subTitleText2 =sf.hasOwnProperty('title2')? esriLang.substitute({ alias: sf.title2.alias||'空', value: feature.attributes[sf.title2.name] || '空' }, '${alias}:${value}'):'';
                        var g = rp.features[i];
            ...
            //渲染模板显示结果
            var tpl = $("#ImportScLpbResultTpl").html();
            $qResultPanel.empty();
            $qResultPanel.append(Mustache.render(tpl, { result: attrs, size: totalSize }));
            var listDataRenderer = new ListDataRenderer({ renderTo: $('#ImportScLpb_result'), type: "scLpbImport", map: map.map(), renderData: attrs });
            listDataRenderer.on('boxDbClick', function (data) {
                doLocation(data.graphic, layerObj);
            });
            listDataRenderer.on('importClick', function (data) {
                ImportClick(data,layerObj);
            });
            ...
            //返回查询界面
            $("#ImportScLpbReturnBtn").on('click', lang.hitch(this, changeState, STATE_QUERY));
            var returnbtn=$("#ImportScLpbReturnBtn");
        }
    }



    function ImportClick(data,layerobject) {
        console.log(data);
        layer.open({
            type: 2,
            title: 'Excel实测楼盘表导入',
            ...
            area: ['960px', '520px'],
            content: _queryConfig.interfaceHost+ _queryConfig.importScLpbAddress + '?ljzmc=' + data.title + '&djh=' + data.subtitle2
        });
    }
    ...
    function getInfoContent(graphic, returnFields) {
        ...
        var data = [];
        var tmpl = $("#ImportScLpbInfoContentTpl").html();
